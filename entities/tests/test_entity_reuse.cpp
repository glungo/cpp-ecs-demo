...
void test_entity_reuse() {
    ...
    assert(!manager.IsEntityActive(entity1) && "Entity 1 should be inactive");
    assert(manager.IsEntityActive(entity2) && "Entity 2 should still be active");
    ...
    assert(manager.IsEntityActive(new_entity) && "New entity should be active");
